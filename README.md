...
PLUGIN_META = {
    ...
    "args": [
        {"name": "flag", "help": "A boolean flag", "type": "flag", "required": False},
        {"name": "path", "help": "Path to file", "type": "string", "required": True},
    ],
}
```
...
```bash
# Top-level invocation (preferred)
python3 scripts/tool.py hello --name Mustafa

# Or via the `plugins` prefix (equivalent)
...
## 12. Plugins (Dynamic CLI extensions)

This project supports dynamic CLI plugins discovered at runtime under `scripts/plugins/`.
Plugins allow you to extend the `tool.py` dispatcher without modifying core code. The
dispatcher will load a plugin named `foo` when a module exists at `scripts/plugins/foo.py`
or `scripts/plugins/foo/__init__.py` and that module implements a `main(argv)` entrypoint.
...
-- Plugins may expose a top-level `PLUGIN_META` dictionary that the tooling will read without importing the
    module. If `PLUGIN_META` is absent, the module docstring is used as the description.
-- Recommended `PLUGIN_META` shape:
...
PLUGIN_META = {
    ...
    "args": [
        {"name": "--name", "help": "Person's name", "required": False, "default": "world"}
    ]
}
```

The `tool plugins describe <name>` command returns the plugin metadata as JSON. The TUI uses
this metadata to populate the Plugins tab (description, args placeholder and quick-run buttons).
