...
inline void _row(std::ostream& os, std::string_view label, std::string_view value,
                 int col = 22)
{
    os << "  " << label;
    for (int i = static_cast<int>(label.size()); i < col; ++i) os << ' ';
    os << ": " << value << '\n';
}
...
inline std::string to_string(
    ...
    std::string_view build_type,     std::string_view library_type,
    std::string_view architecture,   std::string_view compiler_id,
    std::string_view compiler_version, std::string_view cmake_version,
    std::string_view build_timestamp)
{
    std::ostringstream oss;
    print_all(oss, project_name, project_version, git_branch, git_hash,
              git_describe, git_dirty, build_type, library_type, architecture,
              compiler_id, compiler_version, cmake_version, build_timestamp);
    return oss.str();
}
...
#define BUILD_INFO_VERSION_LINE(NS)          \
    (std::string(NS::project_name) + " v" + std::string(NS::project_version) \
     + " (" + std::string(NS::git_branch) + "@" \
     + std::string(NS::git_hash).substr(0, 7) + ")")
