...
function(enable_static_analysis)
    ...
    # Clang Static Analyzer (scan-build / clang --analyze)
    if(ENABLE_CLANG_ANALYZER)
        find_program(SCAN_BUILD_PATH scan-build)
        find_program(CLANG_PATH clang)
        if(SCAN_BUILD_PATH)
            set(PROJECT_CLANG_ANALYZER_TOOL "${SCAN_BUILD_PATH}" PARENT_SCOPE)
            set(PROJECT_CLANG_ANALYZER_USE_SCANBUILD TRUE PARENT_SCOPE)
            message(STATUS "[StaticAnalyzers] Found scan-build: ${SCAN_BUILD_PATH}")
        elseif(CLANG_PATH)
            set(PROJECT_CLANG_ANALYZER_TOOL "${CLANG_PATH}" PARENT_SCOPE)
            set(PROJECT_CLANG_ANALYZER_USE_SCANBUILD FALSE PARENT_SCOPE)
            message(STATUS "[StaticAnalyzers] Found clang for --analyze: ${CLANG_PATH} (scan-build not found)")
        else()
            message(AUTHOR_WARNING "ENABLE_CLANG_ANALYZER set but neither scan-build nor clang were found.")
        endif()
    endif()
